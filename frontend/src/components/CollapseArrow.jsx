...
// section away: a left-pointing ◀ when open, a + when collapsed (never a
// rotating chevron, so the two glyphs read unmistakably as "close" vs
// "reopen"). `dark` flips it to the light-on-translucent variant used on a
// dark bar (the Notenübersicht year frame).
//
...
export default function CollapseArrow({ collapsed, onClick, dark }) {
  const style = {
    flex: 'none',
